...
interface Semester { label:string; courses:CourseGrade[]; }
...
const SEMESTERS:Semester[]=[
  ...
  {label:'السنة 2 - الفصل 2 (جارٍ)',courses:[
...
function standing(gpa:number){
  if(gpa>=3.7)return{label:'ممتاز',color:'text-emerald-600'};
  if(gpa>=3.0)return{label:'جيد جداً',color:'text-blue-600'};
  if(gpa>=2.0)return{label:'جيد',color:'text-yellow-600'};
  if(gpa>=1.0)return{label:'مقبول',color:'text-orange-600'};
  return{label:'راسب',color:'text-red-600'};
}
...
const Grades:React.FC=()=>{
  ...
  const handlePDF=async()=>{
    ...
    doc.setFontSize(10);
    doc.text(`Cumulative GPA: ${cumGPA.toFixed(2)} | Standing: ${st.label}`,14,26);
    SEMESTERS.forEach((s,i)=>{
      if(i>0)doc.addPage();
      doc.setFontSize(12);
      doc.text(s.label,14,i===0?36:18);
      autoTable(doc,{
        startY:i===0?40:24,
  ...
  return(
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      <PageHeader title="الدرجات والنتائج" subtitle="سجلك الأكاديمي الكامل" breadcrumb="البوابة / الدرجات"/>
      <div className="container mx-auto px-4 py-10">
        {/* Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            {label:'المعدل التراكمي',value:cumGPA.toFixed(2),icon:<TrendingUp size={20}/>,bg:'bg-blue-50 dark:bg-blue-900/30',ic:'text-blue-600'},
            {label:'المستوى الأكاديمي',value:st.label,icon:<Award size={20}/>,bg:'bg-purple-50 dark:bg-purple-900/30',ic:'text-purple-600'},
            {label:'الساعات المكتملة',value:`${allDone.reduce((s,c)=>s+c.creditHours,0)}h`,icon:<BookOpen size={20}/>,bg:'bg-emerald-50 dark:bg-emerald-900/30',ic:'text-emerald-600'},
            {label:'معدل الفصل',value:semGPA>0?semGPA.toFixed(2):'-',icon:<TrendingUp size={20}/>,bg:'bg-orange-50 dark:bg-orange-900/30',ic:'text-orange-600'},
          ].map(card=>(
            <motion.div key={card.label} whileHover={{y:-3}} className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-800">
        ...
        <div className="flex gap-2 flex-wrap mb-6">
          {SEMESTERS.map((s,i)=>(
            <button key={i} onClick={()=>setActiveTab(i)} className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${i===activeTab?'bg-blue-600 text-white shadow-lg shadow-blue-500/20':'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700'}`}>{s.label}</button>
          ))}
        </div>
        {/* Table */}
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm overflow-hidden border border-slate-100 dark:border-slate-800">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white">{sem.label}</h3>
              <p className="text-slate-500 text-sm">المعدل: <span className="font-bold text-blue-600">{semGPA>0?semGPA.toFixed(2):'جارٍ'}</span></p>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="bg-slate-50 dark:bg-slate-800/50">
                {['الكود','المادة','الساعات','نصف الفصل','النهائي','العملي','الإجمالي','التقدير'].map(h=>(
                  <th key={h} className="px-4 py-3 text-start text-xs font-bold text-slate-500 uppercase">{h}</th>
                ))}
              </tr></thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {sem.courses.map((c,i)=>(
                  <motion.tr key={c.code} initial={{opacity:0,x:-10}} animate={{opacity:1,x:0}} transition={{delay:i*0.05}} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3"><span className={`text-xs font-bold px-2.5 py-1 rounded-full ${GRADE_COLORS[c.grade]||GRADE_COLORS['N/A']}`}>{c.grade}</span></td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
