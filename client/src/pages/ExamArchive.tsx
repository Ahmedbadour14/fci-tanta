...
const EXAMS: Exam[] = [
  { id: '1', course: 'Algorithms', code: 'CS301', department: 'CS', year: 2024, type: 'final', professor: 'Dr. Ahmed Mohamed', downloads: 342, pdfUrl: '#' },
  { id: '2', course: 'Data Structures', code: 'CS201', department: 'CS', year: 2024, type: 'midterm', professor: 'Dr. Sara Ali', downloads: 289, pdfUrl: '#' },
  { id: '3', course: 'Database Systems', code: 'IS301', department: 'IS', year: 2023, type: 'final', professor: 'Dr. Mahmoud Khaled', downloads: 415, pdfUrl: '#' },
  { id: '4', course: 'Computer Networks', code: 'IT301', department: 'IT', year: 2024, type: 'final', professor: 'Dr. Noha Hassan', downloads: 198, pdfUrl: '#' },
  { id: '5', course: 'Software Engineering', code: 'SE301', department: 'SE', year: 2023, type: 'midterm', professor: 'Dr. Amro Salem', downloads: 156, pdfUrl: '#' },
  { id: '6', course: 'Operating Systems', code: 'CS302', department: 'CS', year: 2023, type: 'final', professor: 'Dr. Yasser Helmy', downloads: 378, pdfUrl: '#' },
  { id: '7', course: 'Machine Learning', code: 'CS401', department: 'CS', year: 2024, type: 'midterm', professor: 'Dr. Mona Ibrahim', downloads: 512, pdfUrl: '#' },
  { id: '8', course: 'Information Systems Analysis', code: 'IS201', department: 'IS', year: 2023, type: 'final', professor: 'Dr. Hesham Farouk', downloads: 234, pdfUrl: '#' },
  { id: '9', course: 'Web Development', code: 'IT201', department: 'IT', year: 2024, type: 'final', professor: 'Dr. Shereen Mostafa', downloads: 445, pdfUrl: '#' },
  { id: '10', course: 'Design Patterns', code: 'SE401', department: 'SE', year: 2023, type: 'midterm', professor: 'Dr. Rania Yousef', downloads: 167, pdfUrl: '#' },
  { id: '11', course: 'Computer Vision', code: 'CS402', department: 'CS', year: 2022, type: 'final', professor: 'Dr. Sara Ali', downloads: 298, pdfUrl: '#' },
  { id: '12', course: 'Cloud Computing', code: 'IT402', department: 'IT', year: 2022, type: 'midterm', professor: 'Dr. Shereen Mostafa', downloads: 189, pdfUrl: '#' },
];
      ...
      <div className="container mx-auto px-4 py-10">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm mb-8 flex flex-wrap gap-3 items-center">
          ...
          <select value={dept} onChange={e => setDept(e.target.value)}
            ...
            {DEPTS.map(d => <option key={d} value={d}>{d === 'All' ? t('exams.allDepts') : d}</option>)}
          </select>

          <select value={year} onChange={e => setYear(e.target.value === 'All' ? 'All' : Number(e.target.value))}
            className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white text-sm outline-none">
            {YEARS.map(y => <option key={y} value={y}>{y === 'All' ? t('exams.allYears') : y}</option>)}
          </select>
          ...
          <select value={type} onChange={e => setType(e.target.value)}
            ...
            {TYPES.map(tp => (
              <option key={tp} value={tp}>
                {tp === 'All' ? t('exams.examType') : tp === 'midterm' ? t('exams.midterm') : t('exams.final')}
              </option>
            ))}
          </select>
          ...
          <div className="flex items-center gap-2 text-slate-500 text-sm ms-auto">
            <Filter size={14} />
            <span>{filtered.length} {t('exams.results')}</span>
          </div>
        </div>
              ...
              <motion.div key={exam.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${exam.type === 'final'
                      ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'
                      : 'bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400'}`}>
                      {exam.type === 'final' ? t('exams.final') : t('exams.midterm')}
                    </span>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => handleDownload(exam.id)}
                      ...
                      <Download size={13} /> {t('exams.downloadPDF')}
                    </button>
                  </div>
                </div>
              </motion.div>
